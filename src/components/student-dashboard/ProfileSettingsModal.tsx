                  ...
                  <div className="flex items-center gap-5 p-4 bg-slate-50 rounded-2xl border border-slate-200">
                    ...
                    <div className="space-y-2 flex-1">
                      ...
                      <div className="flex flex-col gap-2">
                        <div className="flex items-center gap-2">
                          <label className="flex items-center gap-1.5 px-3 py-1.5 bg-white border border-slate-300 rounded-lg text-xs font-semibold text-slate-700 hover:bg-slate-50 cursor-pointer transition-colors shadow-sm">
                          ...
                          <button
                            ...
                            className="px-3 py-1.5 bg-emerald-50 border border-emerald-200 rounded-lg text-xs font-semibold text-emerald-700 hover:bg-emerald-100 cursor-pointer transition-colors shadow-sm"
                          >
                            {isEditingPhoto ? 'Hide URL Input' : 'Use Image URL'}
                          </button>
                        </div>
                        ...
                        {isEditingPhoto && (
                          <input
                            ...
                            className="w-full text-xs px-3 py-1.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 mt-1"
                          />
                        )}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div className="sm:col-span-2">
                      <label className="block text-xs font-bold text-slate-700 mb-1 flex items-center justify-between">
                        <span>Personal Google Account Email</span>
                        {profileData.googleEmailId && (
                          <span className="text-[10px] text-amber-600 font-semibold bg-amber-50 px-2 py-0.5 rounded border border-amber-200">
                            Cannot change after setting. Contact Admin to update.
                          </span>
                        )}
                      </label>
                      <input
                        type="email"
                        value={formData.googleEmailId || ''}
                        onChange={(e) => {
                          if (!profileData.googleEmailId) {
                            handleInputChange('googleEmailId', e.target.value);
                          }
                        }}
                        disabled={!!profileData.googleEmailId}
                        placeholder="e.g. your.personal@gmail.com"
                        className={`w-full px-3.5 py-2 text-xs bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 font-semibold ${
                          !!profileData.googleEmailId ? 'opacity-70 cursor-not-allowed bg-slate-100' : ''
                        }`}
                      />
                    </div>
